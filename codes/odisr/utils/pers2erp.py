...
class Perspective:
    def __init__(self, img_name, FOV, THETA, PHI):
        self._img = cv2.imread(img_name, cv2.IMREAD_COLOR)[256:256*3, 512:1024, :]
    ...
    def GetEquirec(self, height, width):
        ...
        return persp, mask, lon_map, lat_map


def pers2erp(_img, PHI, wFOV=90, THETA=0, erp_size=[1024, 2048]):
    # [h, w, c]
    [_height, _width, _] = _img.shape
    [height, width] = erp_size
    hFOV = float(_height) / _width * wFOV
    ...
    persp = cv2.remap(_img, lon_map.astype(np.float32), lat_map.astype(np.float32), cv2.INTER_CUBIC,
                      borderMode=cv2.BORDER_WRAP)
